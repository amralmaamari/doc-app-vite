...
const MyAppointments = () => {
    ...
    const getUserAppointments = async () => {
        try {
            ...
            const { data } = await axios.get(backendUrl + '/api/AppointmentApi/user/appointments', { headers: { Authorization: `Bearer ${token}` }})
            
            
            if(data.success){
                setAppointments(data.appointments.reverse())
            }else{
                toast.error("Error appointments not fetch");
            }
    ...
    const cancelAppointment = async (appointmentId) => {
        ...
        try {
            ...
            const { data } = await axios.delete(
                `${backendUrl}/api/AppointmentApi/CancelAppointment/${appointmentId}`,
                {
                ...
                {appointments.map((item, index) => {
                    ...
                    return (
                        <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                            <div>
                                <img className='w-36 bg-[#EAEFFF]' src={docData.Image} alt="" />
                            </div>
                            <div className='flex-1 text-sm text-[#5E5E5E]'>
                                <p className='text-[#262626] text-base font-semibold'>{docData.Name}</p>
                                <p>{docData.Speciality}</p>
                                <p className='text-[#464646] font-medium mt-1'>Address:</p>
                                <p className=''>{docData.AddressLine1}</p>
                                <p className=''>{docData.AddressLine2}</p>
                                <p className='mt-1'>
                            ...
                            <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                                ...
                                {!item.cancelled && !item.isCompleted && (
                                    <button onClick={() => cancelAppointment(item.appID)} className='text-[#696969] sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancel appointment</button>
                                )}
                                {item.cancelled && !item.isCompleted && (
